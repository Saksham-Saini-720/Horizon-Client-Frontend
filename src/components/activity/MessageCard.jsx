...
const MessageCard = memo(({ message }) => {
  ...
  // conversationId se navigate karo
  const handleMessageClick = useCallback(() => {
    const convId = message.conversationId || message.id;
    navigate(`/chat/${convId}`);
  }, [message.conversationId, message.id, navigate]);
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        {/* Agent Photo */}
        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center text-white text-[18px] font-bold font-interflex-shrink-0 overflow-hidden">
          {agent?.avatar ? (
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-[14px] font-bold text-primary font-playfair mb-1">
            {agent?.name}
          </h3>
          <p className="text-[10px] text-amber-600 font-inter mb-1">
            Re: {property?.title}
          </p>
        ...
        <div className="flex flex-col items-end gap-3 flex-shrink-0">
          <p className="text-[10px] text-gray-400 font-inter">{timestamp}</p>
